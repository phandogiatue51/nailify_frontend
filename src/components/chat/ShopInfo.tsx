...
import { ChevronLeft, Phone, Mail, ShieldCheck, MapPin, Info } from "lucide-react";
        ...
        <div className="bg-white rounded-[2.5rem] p-6 shadow-xl shadow-[#950101]/5 border border-slate-50">
          <div className="flex flex-col items-center text-center">
            ...
            <p className="text-slate-500 text-sm font-medium mt-2 leading-relaxed px-4">
              {shop.description ?? "Expert nail artistry tailored to your unique style."}
            </p>
          </div>
          ...
          <div className="mt-6 bg-[#950101] rounded-[2rem] p-6 text-white shadow-lg shadow-[#950101]/20">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-white/10 rounded-2xl p-3">
                ...
                <p className="font-black text-sm uppercase">{shop.isActive ? "Active" : "Inactive"}</p>
              </div>
              <div className="bg-white/10 rounded-2xl p-3">
                <p className="text-[10px] font-bold opacity-60 uppercase">Joined</p>
                <p className="font-black text-sm uppercase">{new Date(shop.createdAt).toLocaleDateString()}</p>
              </div>
            </div>
          </div>
        )}
...
function LoadingState() {
  return (
    <div className="flex flex-col items-center justify-center h-screen bg-white">
      <div className="w-12 h-12 border-4 border-[#FFCFE9] border-t-[#950101] rounded-full animate-spin mb-4" />
      <p className="text-xs font-black text-slate-400 uppercase tracking-widest">Loading Salon...</p>
    </div>
  );
}
...
function NotFoundState() {
  return (
    <div className="flex flex-col items-center justify-center h-screen bg-white p-8 text-center">
      ...
      <h2 className="text-xl font-black text-slate-800 uppercase tracking-tight">Shop Not Found</h2>
      <p className="text-slate-400 text-sm mt-2">The salon you're looking for might have closed or changed addresses.</p>
    </div>
  );
}