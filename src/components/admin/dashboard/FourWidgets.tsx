...
import { DollarSign, Users, FileText, Star, Loader2, ArrowUpRight } from "lucide-react";
...
const StatCard = ({ title, value, icon: Icon, color, subtext }: any) => (
    <div className="group relative overflow-hidden bg-white rounded-[2rem] p-7 border-2 border-slate-50 transition-all duration-500 hover:border-[#950101] hover:shadow-2xl hover:shadow-[#950101]/10">
        ...
        <div className={`absolute -right-4 -bottom-4 w-24 h-24 rounded-full opacity-[0.1] transition-opacity ${color.split(' ')[0]}`} />
    </div>
);
    ...
    if (isLoading) {
        return (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {[...Array(4)].map((_, i) => (
                    <div key={i} className="h-40 rounded-[2rem] bg-slate-50 animate-pulse flex items-center justify-center">
